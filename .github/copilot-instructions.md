# todo Development Guidelines

Auto-generated from all feature plans. Last updated: 2026-04-12

## Active Technologies
- TypeScript 5.x with React 18.x + React, React DOM, Chakra UI 2.x, Vite 5.x (002-portuguese-internationalization)
- localStorage (keys unchanged, only displayed values affected) (002-portuguese-internationalization)
- TypeScript 5.x with React 18.x (strict mode) + React, React DOM, Chakra UI 2.x, Vite 5.x (002-portuguese-internationalization)
- localStorage with JSON serialization (002-portuguese-internationalization)

- TypeScript 5.x, React 18.x + React, React DOM, Chakra UI, TypeScript, Vite (build tool) (001-shopping-list-dashboard)

## Project Structure

```text
backend/
frontend/
tests/
```

## Commands

npm test && npm run lint

## Code Style

TypeScript 5.x, React 18.x: Follow standard conventions

## Recent Changes
- 002-portuguese-internationalization: Added TypeScript 5.x with React 18.x (strict mode) + React, React DOM, Chakra UI 2.x, Vite 5.x
- 002-portuguese-internationalization: Added TypeScript 5.x with React 18.x + React, React DOM, Chakra UI 2.x, Vite 5.x

- 001-shopping-list-dashboard: Added TypeScript 5.x, React 18.x + React, React DOM, Chakra UI, TypeScript, Vite (build tool)

<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
